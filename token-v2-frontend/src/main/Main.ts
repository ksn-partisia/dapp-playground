...
import { getTokenV2Api, isConnected, setContractAddress } from "./AppState";
import {
...
function contractAddressClick() {
  ...
  if (address === undefined) {
    ...
    currentAddress.innerHTML = `MPC20-v2 Address: ${address}`;
...
function transferAction() {
  ...
  if (isConnected() && api !== undefined) {
    ...
    api
      .transfer(to.value, new BN(amount.value, 10))
